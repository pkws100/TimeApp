...
├── prepare-1.0-build.sh
├── build-all.sh
└── dist/  generated, ignored binaries and SHA256SUMS
```

Build firmware 1.1 with `cd 1.1 && pio run`. Before every operational 1.0
rollback build or upload, run `PIO_CMD=pio sh prepare-1.0-build.sh` from this
directory; it installs the verified direct library versions without changing
the frozen 1.0 files. Then use `cd 1.0 && pio run` or `pio run -t upload`.
`./build-all.sh` performs the same preparation automatically and produces
`dist/pkws-time-terminal-1.0.bin`, `pkws-time-terminal-1.1.bin` and `SHA256SUMS`.
